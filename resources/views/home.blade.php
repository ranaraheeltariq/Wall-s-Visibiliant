    ...
    <div class="content-wrapper pb-0">
                      ...
                      <div class="row">
                        <div class="col-md-8">
                          <div class="input-group input-daterange d-flex align-items-center">
                            <input type="text" name="from" class="form-control{{ $errors->has('from') ? ' form-control-danger' : '' }}" data-date-format='yyyy-mm-dd' value="2012-04-05" placeholder="Select date range">
                            @if ($errors->has('from'))
                            <label class="error mt-2 text-danger" for="$errors->has('from')">{{ $errors->first('from') }}</label>
                            @endif
                            <div class="input-group-addon mx-4">to</div>
                            <input type="text" name="to" class="form-control{{ $errors->has('to') ? ' form-control-danger' : '' }}" data-date-format='yyyy-mm-dd' value="2012-04-19" placeholder="Select date range">
                            @if ($errors->has('to'))
      ...
      <div class="row">
        <div class="col-lg-6 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Walls Visibility</h4>
...
@parent
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        var visibility = {!! json_encode(array_count_values($array)) !!};
        var second = new Array();
        $.each(visibility, function(keys, index){
          second.push({label: keys, value: index});
        });
        if ($("#morris-donut-example").length) {
          Morris.Donut({
            element: 'morris-donut-example',
            colors: ['#76C1FA', '#F36368', '#63CF72', '#FABA66', '#FF0000', '#80DEEA', '#F9A825', '#BA68C8', '#FFEB3B', '#283747'],
            data:second
          });
        }
      });
