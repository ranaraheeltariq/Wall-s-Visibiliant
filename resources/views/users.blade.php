									...
									<div class="col-sm-12 col-lg-12 d-lg-flex">
										...
										<div class="wrapper pl-lg-4">
											<div class="wrapper d-flex align-items-center">
												<h4 class="mb-0 font-weight-semibold"><a href="{{route('profile',$user->id)}}">{{ $user->name }}</a></h4>
												<div class="badge badge-secondary text-dark mt-2 ml-2">{{ $user->designation }}</div>
											</div>
											<div class="wrapper d-flex align-items-center pt-2 font-weight-semibold text-muted">
												<p class="mb-0 text-muted">{{ $user->email }} {{ $user->mobile ? '- '.$user->mobile : '' }}</p>
											</div>
											<div class="wrapper d-flex align-items-center pt-2 font-weight-semibold text-muted">
												<p class="mb-0 text-muted">{{ $user->region_id ? 'Region: '.$user->region->name.',' : '' }} {{ $user->conc_id ? 'Conc: '.$user->conc->name.',' : '' }} {{ $user->territory_id ? 'Territory: '.$user->territory->name.',' : '' }}</p>
											</div>
